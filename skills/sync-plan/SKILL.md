...
the change — and those stale docs are what the next `/plan-change` hands its designers, what
`/plan-package` hands the next package as its upstream, and what `/implement-section` ranks in
...
3. **Verify each section shipped.** A section counts as shipped when all three hold: the files
   ...
   the plan's shape, not re-reviewing its quality, which is `/review-section`'s job.

   Sections divide into three, and partial plans are normal — implementation happens one section
   at a time, so most `/sync-plan` runs will find some sections pending:
   ...
   - **Not implemented** → skip it. Do not update its canonical docs; that would put a
     ...
     as pending, and note that `/sync-plan $plan` should be run again once it ships.
...
6. **Update `docs/packages/<pkg>/surface.md` and `interface.md`** where the delta's
   **Interface: <pkg>** group names them and the surface code has actually changed (check
   ...
   snapshot with the same greps `/plan-change` uses, and for any consumer that the plan did not
...
8. **Reconcile `docs/packages/<pkg>/integration.md`.** Fold in cross-section resolutions the
   ...
   ever ran `/plan-change` — create it now from the plan's integration doc plus the canonical
   designs.
...
9. **Record what you synced.** Append to `docs/plans/synced.md` (create if absent), one line per
   ...
   This is what makes re-running safe. Without it, a second `/sync-plan $plan` after the
...
11. **File what you could not fix.** Where the shipped code contradicts the plan in a way that
    ...
    what the next `/implement-section` actually reads.

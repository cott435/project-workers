...
The lint thresholds are **not** here. They live in `.claude/pyproject-lint-config.toml`, which
...
# --- merge .claude/pyproject-lint-config.toml here: [tool.ruff*], [tool.pylint*] ---
...
[project.scripts]
# <pkg>-<verb> = "<pkg>.cli:<function>"   — added by /finalize-package; the module must live
...
#    Written by /finalize-package from the package contract's Sections table.
[[tool.importlinter.contracts]]
...
**Growing the block.** The first `/implement-section` of a package adds the package to
`root_packages` and to contract 1 in the position `docs/architecture.md` gives, and adds
contract 3 for that package. `/finalize-package` adds contract 2. Never list a package that
has no code yet.
      ...
      handlers:
        python:
          ...
          options:
            ...
            show_source: false
nav:
  - Home: index.md                    # stub written at scaffold; regenerated by /finalize-project
  - Architecture: architecture.md
  - Decisions: decisions.md
  - API: []                           # /finalize-package appends "- <pkg>: api/<pkg>.md" per package
```
...
`Architecture`, and `Decisions`; each `/finalize-package` adds `docs/api/<pkg>.md` (one
`::: <module>` block per providing module in `interface.md`, plus `::: <pkg>.cli` and the
pipeline modules) and its `API` nav entry; `/finalize-project` regenerates `index.md` and
